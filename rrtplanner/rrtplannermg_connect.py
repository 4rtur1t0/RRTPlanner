"""
    ...
    An extend operation is carried out only on a single-tree variant and tries to connect any of the goals with the last
    added qnew
    ...
    algorithm always returns a solution:
        a) If any of the goals is reached, the path that connects the start and goal is returned.
...
class RRTPlannerMGConnect():
    ...
    def build_rrt(self):
        ...
        # intentar aquí una operación de connect
        for k in range(self.max_nodes):
            print('Iteration: ', k)
            qrand = self.random_config()
            qnew, result = self.extend(qrand)
            if result == ADVANCED:
                # try to connect the goal
                qnew, result = self.connect()
    ...
    def add_vertex(self, parent_id, coordinates, reached_goal=False):
        # new id depending on the number of nodes
        id = len(self.tree)
        node = Node(id=id, parent_id=parent_id, coordinates=coordinates, reached_goal=reached_goal)
    ...
    def extend(self, qrand):
        ...
        # if self.distance(qnear, qnew) < self.epsilon:
        #     return qnew, REACHED
        if self.in_obstacle(qnew):
            return qnew, TRAPPED
        if self.distance(qnew, self.goal) < self.epsilon:
            self.add_vertex(parent_id=inear, coordinates=qnew, reached_goal=True)
            return qnew, GOAL_REACHED
        self.add_vertex(parent_id=inear, coordinates=qnew, reached_goal=False)
        return qnew, ADVANCED

    def connect(self):
        """
        The connect operation of the tree.
        We are trying to connect qnew to the goal.
        :param qrand:
        :return:
        """
        while True:
            qnew, result = self.extend(self.goal)
    ...
    def get_path(self):
        ...
        if solution_node is None:
            solution_node = self.nearest_neighbour(self.goal)
...
class Node():
    ...
    """
    def __init__(self, id, parent_id, coordinates, reached_goal=False):
        self.id = id
